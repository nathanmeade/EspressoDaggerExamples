...
@Inject
constructor(private val jsonUtil: JsonUtil): ApiService{
    ...
    override suspend fun getBlogPosts(category: String): List<BlogPost> {
        ...
        val blogs = Gson().fromJson<List<BlogPost>>(rawJson, object : TypeToken<List<BlogPost>>(){}.type)
        val filteredBlogPosts = blogs.filter { blogPost -> blogPost.category.equals(category) }
        delay(networkDelay)
    ...
    override suspend fun getCategories(): List<Category> {
        ...
        val categories = Gson().fromJson<List<Category>>(rawJson, object : TypeToken<List<Category>>(){}.type)
        return categories
    }
